...
function useResLiqCod(getId) {
  return useFetch(
    () =>
      `${apiBase.value}/api/view/planillaRetCPA?${getId()}&sort={"IdRep":"asc","Orden":"asc","Periodo":"asc"}`
  )
}
...
function exportFile() {
  ...
  const totalesTabla = [null, null, null, null, null, null, null, totImporte.value.totImp, null]
